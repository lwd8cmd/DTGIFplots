...
scan = 2# what scan to process (txt file in path data/scan/)
	...
	for superlayer in [1,2,3]:
		for layer in [1,2,3,4]:
			for wire in [0,1,3]:
				...
				# save plot
				plt.tight_layout()
				filename = 'data/{s}/scan{s}SL{sl}L{l}{w}.png'.format(s=scan, sl=superlayer, l=layer, w=wire_strs[wire])
...
if 1:# 2d plot
	...
	for attenuation_i, attenuation in enumerate(np.unique(attenuations)):
		...
		for wire in [0,1,3]:
			...
			plt.tight_layout()
			filename = 'data/{s}/scan{s}2d1{w}att{a}.png'.format(s=scan, w=wire_strs[wire], a=attenuation_i)
			print("Saving {}".format(filename))
			plt.savefig(filename, bbox_inches='tight')
			plt.clf()
		...
		for wire in [0,1,3]:
			...
			plt.tight_layout()
			filename = 'data/{s}/scan{s}2d2{w}att{a}.png'.format(s=scan, w=wire_strs[wire], a=attenuation_i)
			print("Saving {}".format(filename))
			plt.savefig(filename, bbox_inches='tight')
			plt.clf()
		...
		if 1:
			...
			plt.tight_layout()
			filename = 'data/{s}/scan{s}2d3att{a}.png'.format(s=scan, w=wire_strs[wire], a=attenuation_i)
			print("Saving {}".format(filename))
			plt.savefig(filename, bbox_inches='tight')
			plt.clf()